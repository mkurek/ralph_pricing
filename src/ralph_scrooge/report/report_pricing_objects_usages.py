...
class PricingObjectsUsagesReport(BasePluginReport):
    """
    Report with usages of resources (usage types) by service environments
    per days
    """
    cache_section = 'pricing-services-usages-report'
    ...
    def get_data(
        ...
        **kwargs
        ...
        service_environments = [service_environments] if service_environments else cls._get_services_environments(is_active)
        ...
        # TODO: select only daily pricing objects for SE
        pricing_objects = PricingObject.objects.filter(service_environment__in=service_environments)
    ...
    def get_header(cls, start, end, usage_types, **kwargs):
        ...
        for usage_type in usage_types:
            usage_type_header = plugin_runner.run(
                ...
                usage_type=usage_type,
                type='usages_pricing_object_schema',
            )
